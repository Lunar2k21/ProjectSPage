...
const translations = {
    pl: {
        ...
        lmuCommunity: "Profil Społeczności",
        lmuSeason: "Aktualny Sezon",
        lmuResults: "Wyniki (wkrótce)",
        supportPatronite: "Wspieraj na Patronite.pl",
        scrollDown: "↓ Wyniki WRC/Media ↓",
        ...
        errorLoading: "Błąd podczas ładowania danych. Sprawdź połączenie z internetem.",
        noSheets: "Arkusz nie zawiera danych",
    ...
    en: {
        ...
        lmu: "Le Mans Ultimate",
        lmuCommunity: "Community Profile",
        lmuSeason: "Current Season",
        lmuResults: "Results (coming soon)",
        supportPatronite: "Support on Patronite.pl",
        scrollDown: "↓ WRC Results/Media ↓",
        latestContent: "Latest Content",
        videoBtn: "Video",
        galleryBtn: "Gallery",
        resultsTable: "Results Table",
        season13: "Main League",
        pateraCup: "PaTera Cup",
        loadingResults: "Loading results...",
        noFile: "Unable to load results from Google Sheets.",
        errorLoading: "Error loading data. Check your internet connection.",
        noSheets: "Sheet contains no data",
...
function updateLanguage(lang) {
    ...
    if (columnTitles[3]) columnTitles[3].textContent = t.lmu;
    ...
    if (linkBoxes[8]) linkBoxes[8].textContent = t.lmuCommunity;
    if (linkBoxes[9]) linkBoxes[9].textContent = t.lmuSeason;
    if (linkBoxes[10]) linkBoxes[10].textContent = t.lmuResults;
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    function nextSlide() {
        ...
        if (blurBg && slideImages[currentSlide]) {
            blurBg.style.backgroundImage = `url(${slideImages[currentSlide]})`;
        }
    ...
    if (galleryImages.length > 0 && galleryGrid) {
        ...
        galleryImages.forEach((imagePath, index) => {
            const img = document.createElement('img');
            img.src = imagePath;
            img.alt = `Rally Gallery Image ${index + 1}`;
    ...
    // FIX: Added theme toggle hiding when modal opens
    window.openImageModal = function(imgElement) {
        ...
        // Hide theme toggle when modal is open (fallback for browsers without :has() support)
        if (themeToggleBtn) {
    ...
    // FIX: Added theme toggle showing when modal closes
    window.closeImageModal = function() {
        ...
        // Show theme toggle when modal is closed
        if (themeToggleBtn) {
    ...
    // Google Sheets Excel Loading
    let currentWorkbook = null;
    
    // Load main file on page load
    loadExcelFile('https://docs.google.com/spreadsheets/d/1fLWQQbc7HStSIGq3NQbhQQgR-bZWzC19oCcl4MD0iA8/export?format=xlsx');
    ...
    function loadExcelFile(googleSheetsUrl) {
        const t = translations[currentLang];
        
        // Show loading message
        document.getElementById('resultsTable').innerHTML = 
            `<p class="loading-text">${t.loadingResults}</p>`;
        ...
        fetch(googleSheetsUrl)
            ...
            .catch(error => {
                ...
                document.getElementById('resultsTable').innerHTML = 
                    `<p class="loading-text">${t.noFile}<br><br>${t.errorLoading}</p>`;
            });
    }
